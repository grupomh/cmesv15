...
class SaleCommissionAnalysisReport(models.Model):
    ...
    start_date = fields.Date("Desde",dafault=fields.Date.today())
    end_date = fields.Date("Hasta",dafault=fields.Date.today())
    user_id = fields.Many2one("res.users",string="Vendedores")
    
    @api.constrains('start_date', 'end_date')
    def _check_dates(self):
        if self.filtered(lambda c: c.end_date and c.start_date > c.end_date):
            raise ValidationError(_('La fecha de inicio debe ser menor que la fecha de finalización.'))    
