...
class AccountFollowupReport(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        ...
        headers = [{},
                   ...
                   {'name': _('FEL NO'), 'style': 'text-align:right; white-space:nowrap;'},
                   {'name': _('FEL SERIE'), 'style': 'text-align:right; white-space:nowrap;'},
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for currency, aml_recs in res.items():
            ...
            for aml in aml_recs:
                ...
                if aml.move_id.fel_no:
                    move_line_name = aml.move_id.fel_no
                if self.env.context.get('print_mode'):
                ...
                if aml.move_id.invoice_origin:
                    sale_order = self.env['sale.order'].search([('name', '=', aml.move_id.invoice_origin)])
                    if len(sale_order):
                        invoice_origin = sale_order[0].client_order_ref
                fel_no = False
                if aml.move_id.fel_no:
                    fel_no = aml.move_id.fel_no
                fel_serie = False
                if aml.move_id.fel_serie:
                    fel_serie = aml.move_id.fel_serie
                columns = [
