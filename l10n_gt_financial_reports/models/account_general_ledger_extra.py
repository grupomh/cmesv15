...
class AccountGeneralLedgerReportExtra(models.AbstractModel):
    ...
    def get_init_bal(self, target_date, acc_id, day_domain):
        ...
        date_init = datetime.strptime(str(target_date), '%Y-%m-%d').strftime('%Y-01-01')
        init_bal = False
        account_obj = self.env['account.account'].browse([acc_id])
        if account_obj and account_obj.internal_group in ('income', 'expense'):
            day_domain += [("date", ">=", date_init)]
            init_bal = self.env["account.move.line"].read_group(day_domain, ["account_id", "credit", "debit","date"], ["account_id"], orderby="date asc")
        else:
            init_bal = self.env["account.move.line"].read_group(day_domain, ["account_id", "credit", "debit","date"], ["account_id"], orderby="date asc")
        result = 0
        if init_bal:
            ...
            if "account_" in line["id"]:
                ...
                if is_daily_book:
                    for move in res:
                        ...
                        move_obj = self.env['account.move'].browse([move['move_id'][0]])
                        target_name = "{0} - {1}".format((move_obj.name or ''), (move_obj.partner_id.name if move_obj.partner_id else ''))
    ...
    def _get_reports_buttons(self):
        ...
        res.append({"name": "Libro Mayor Resumido", "action": "general_ledger_wizard", "sequence": 5})
        res.append({"name": "Libro Auxiliar Mayor", "action": "daily_mayor_book_pdf", "sequence": 6})
        return res

    def mayor_book_pdf(self, options):
        report = self.env.ref("l10n_gt_financial_reports.general_ledger_report")
        if self._context.get("is_daily_book", False):
    ...
    def daily_mayor_book_pdf(self, options):
        holder = self.with_context(is_daily_book=True).mayor_book_pdf(options)
        return holder
    
    def general_ledger_wizard(self, options):
