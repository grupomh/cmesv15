...
class purchase_order_line(models.Model):
    ...
    def _prepare_stock_moves(self, picking):
        ...
        if template:
            template[0].update({'picking_type_id': self.picking_type_id.id,
    ...
    def _create_or_update_picking(self):
        for line in self:
            if line.product_id and line.product_id.type in ('product', 'consu'):
                ...
                pickings = line.order_id.picking_ids.filtered(lambda x: x.state not in ('done', 'cancel') and x.location_dest_id.usage in ('internal', 'transit') and x.picking_type_id.id == line.picking_type_id.id)
