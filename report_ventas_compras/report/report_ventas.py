...
class ReportSaleBook(models.AbstractModel):
    ...
    # @api.multi
    def generate_records(self, data, company_id, rows_per_page, cf=False):
        result = []
        ...
        facturas = self.env['account.move'].search(
            [('state', 'in', ['posted', 'cancel']),
             ...
             ('date', '>=', date_from),
             ('date', '<=', date_to),
             ('company_id', '=', empresa.id)], order='invoice_date,force_name')
...
class ReportSaleBookCF(ReportSaleBook):
    ...
    def generate_records(self, data, company_id, rows_per_page, cf=False):
        ...
        for row in result:
            ...
            print(row['bienes_gravados'])
            print(row['servicios_gravados'])
