...
class WizardVentasCompras(models.TransientModel):
    ...
    #     if company.country_id.name == 'Guatemala':
    #         return self.env.ref('report_ventas_compras.report_sale_book_ab_guatemala').report_action(self, data=datas)
    #     else:
    ...
    #     company = self.env['res.company'].search([('id', '=', data['company_id'][0])])
    #     if company.country_id.name == 'Guatemala':
    #         return self.env.ref('report_ventas_compras.report_purchase_book_ab_guatemala').report_action(self, data=datas)
    #     else:
    #         return self.env.ref('report_ventas_compras.report_purchase_book_ab').report_action(self, data=datas)

    # def print_report_ventas_cf(self):
    #     self.ensure_one()
    #     [data] = self.read()
    #     datas = {
    #         'ids': self.id,
    ...
    #         'form': data,
    #     }
    #     return self.env.ref('report_ventas_compras.report_sale_cf_book_ab').report_action(self, data=datas)
