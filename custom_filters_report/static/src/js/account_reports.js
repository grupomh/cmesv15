...
var accountReportsWidget = AbstractAction.extend({
    ...
    custom_events: {
        'value_changed': function(ev) {
             ...
             self.report_options.comercial = ev.data.comercial;
             self.report_options.products = ev.data.products;
    ...
    render_searchview_buttons: function() {
        ...
        if (this.report_options.partner) {
            if (!this.M2MFilters) {
               ...
               if ('comercial' in this.report_options) {
                   fields['comercial'] = {
                       label: _t('Comercial'),
                       modelName: 'res.users',
                       value: this.report_options.comercial.map(Number),
                   };
               }
                if (!_.isEmpty(fields)) {
        ...
        if (this.report_options.product) {
            if (!this.M2MFilters) {
                var fields = {};
                 if ('products' in this.report_options) {
                    fields['products'] = {
                        modelName: 'product.product',
                        value: this.report_options.products.map(Number),
                    };
                }
                if (!_.isEmpty(fields)) {
                    this.M2MFilters = new M2MFilters(this, fields);
                    this.M2MFilters.appendTo(this.$searchview_buttons.find('.js_account_product_m2m'));
                }
            } else {
                this.$searchview_buttons.find('.js_account_product_m2m').append(this.M2MFilters.$el);
            }
        }
